...
def filter_test(sheet_id,excel,location_string,sheet_name):
    ...
    for filter_num in range(len(locations)):
        ...
        end_col=start_col+3
        if filter_utility(sheet_id,excel[xl_sheet_num].iloc[:,start_col:end_col])==False:
            res=False
    return res
...
def expected_val_test(sheet_id,excel,location_string,sheet_name):
    locations=location_string.aplit(';')
    res=True
...
def test():
    ...
    xl_sheet_count=len(pd.ExcelFile(NameFile.read()+'.xlsx').sheet_names)
    excel=pd.read_excel(NameFile.read() + '.xlsx',list(range(xl_sheet_count)))
    xl_sheet1=excel[0]
